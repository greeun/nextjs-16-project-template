/**
 * startup-banner.ts — 공용 서버 구동 배너
 *
 * 모든 withwiz 서버 프로젝트에서 서버 구동 시 환경설정을 동일한 형식으로 출력한다.
 * url-shortener-mvp 의 instrumentation 배너에서 추출한 표준 구현이며, 각 프로젝트
 * src(또는 루트)에 그대로 복사해 사용한다. 외부 의존성 없음(순수 console).
 *
 * 사용 예:
 *   import { printStartupBanner, mask, status } from './startup-banner';
 *   printStartupBanner({
 *     title: '🚀 My Service',
